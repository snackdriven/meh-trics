...
export class TaskDeadlineNotificationHandler implements EventHandler {
  ...
  private async scheduleNotification(userId: string, notification: {
    title: string;
    message: string;
    taskId: string;
    scheduledFor: Date;
...
export class HabitStreakNotificationHandler implements EventHandler {
  ...
  private async sendStreakCelebration(userId: string, streak: number, habitId: string): Promise<void> {
    const messages = {
    ...
    console.log(`Streak celebration for user ${userId}: ${streak} days - ${messages[streak as keyof typeof messages]}`);
  }
}
