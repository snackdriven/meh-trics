...
export class JournalTaggingHandler implements EventHandler {
  ...
  private generateJournalTags(content: string, type: string): string[] {
    ...
    const positiveWords = ['happy', 'good', 'great', 'amazing', 'wonderful', 'excited', 'accomplished'];
    ...
    if (lowerContent.includes('exercise') || lowerContent.includes('gym') || lowerContent.includes('run')) {
      tags.push('fitness');
    }
