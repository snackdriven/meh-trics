  ...
  test('should navigate to different sections', async ({ page }) => {
    ...
    for (const section of sections) {
      ...
      if (await navLink.isVisible()) {
        ...
        await expect(page.locator('h1, h2, h3', { hasText: new RegExp(section, 'i') })).toBeVisible();
      }
    }
  });
  ...
  test('should handle mood tracking', async ({ page }) => {
    ...
    if (await moodLink.isVisible()) {
      ...
      if (await moodButtons.first().isVisible()) {
        ...
        await expect(page.locator('text=mood', { hasText: /recorded|saved|updated/i })).toBeVisible();
      }
    }
  });
