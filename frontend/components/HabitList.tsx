...
const HabitCard = memo<HabitCardProps>(({
  ...
  const isCompleted = useMemo(() => inputs.count >= habit.targetCount, [inputs.count, habit.targetCount]);
  const completionPercentage = useMemo(() => Math.min((inputs.count / habit.targetCount) * 100, 100), [inputs.count, habit.targetCount]);
  ...
  const handleCountInputChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    onCountChange(habit.id, parseInt(e.target.value) || 0);
  }, [habit.id, onCountChange]);

  const handleNotesInputChange = useCallback((e: React.ChangeEvent<HTMLTextAreaElement>) => {
...
const HabitListComponent = ({
  ...
  const handleCountChange = useCallback((habitId: number, newCount: number) => {
  ...
  const handleNotesBlur = useCallback((habitId: number) => {
    const count = entryInputs[habitId]?.count || 0;
    const notes = entryInputs[habitId]?.notes || "";
    updateHabitEntry(habitId, count, notes);
  }, [entryInputs, updateHabitEntry]);

  const handleHabitUpdated = useCallback((updatedHabit: Habit) => {
