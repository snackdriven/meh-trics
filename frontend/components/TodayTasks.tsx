...
export function TodayTasks({ date }: TodayTasksProps) {
  ...
  const loadTasks = async () => {
    try {
      const res = await backend.task.listDueTasks({ date, includeOverdue: includeOverdue ? 'true' : undefined });
      let list = res.tasks;
      list = list.sort((a, b) => {
  ...
  useEffect(() => { loadTasks(); }, [includeOverdue, sortBy]);
    ...
    <Card>
      <CardHeader className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => setSortBy(sortBy === 'priority' ? 'created' : 'priority')}>
      ...
      <CardContent>
        {selectedIds.length > 0 && (
          <div className="mb-2 flex gap-2 items-center">
            ...
            <Button size="sm" onClick={bulkComplete}><CheckSquare className="h-4 w-4 mr-1" />Complete</Button>
            <Button size="sm" onClick={bulkReschedule}><Clock className="h-4 w-4 mr-1" />Reschedule</Button>
          </div>
        )}
        {tasks.length === 0 ? (
          ...
          <div className="space-y-3">
            {tasks.map(task => (
              <div key={task.id} className="p-3 border rounded-lg flex items-start gap-2">
                <Checkbox checked={selectedIds.includes(task.id)} onCheckedChange={c => toggleSelect(task.id, !!c)} />
