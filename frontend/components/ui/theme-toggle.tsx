    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          {showLabel && (
            <span className="ml-2 text-sm capitalize">{theme === "system" ? "Auto" : theme}</span>
          )}
        </Button>
      </DropdownMenuTrigger>
...
export function ThemeSelector({ className }: { className?: string }) {
  ...
  const themes = [
    ...
    },
    {
      ...
      icon: Monitor,
      preview:
        "bg-gradient-to-br from-[var(--color-background-secondary)] to-[var(--color-background-inverse)] border-[var(--color-border-primary)]",
    },
  ] as const;

  return (
    <div className={cn("space-y-3", className)}>
      <h4 className="text-sm font-medium text-[var(--color-text-primary)]">Choose your theme</h4>
      ...
      <div className="grid grid-cols-3 gap-3">
        {themes.map((themeOption) => {
          ...
          return (
            ...
            >
              <div className={cn("mx-auto mb-2 h-8 w-12 rounded border", themeOption.preview)} />
