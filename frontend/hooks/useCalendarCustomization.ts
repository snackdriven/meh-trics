...
export function useCalendarCustomization() {
  ...
  const getEventBadgeClass = () => {
    ...
    switch (settings.eventBadgeStyle) {
      case 'square': return `${baseClass} rounded-none`;
      case 'pill': return `${baseClass} rounded-full`;
      default: return `${baseClass} rounded`;
    }
  };
  ...
  const getDayNames = () => {
    if (settings.startOfWeek === 'monday') {
      ...
      return [
        ...settings.customLabels.dayNames.slice(1),
        settings.customLabels.dayNames[0]
      ];
    }
