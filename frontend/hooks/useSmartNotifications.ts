...
export function useSmartNotifications() {
  ...
  const showNotification = useCallback(async (notification: SmartNotification) => {
    if (!isSupported || !registration || permission !== 'granted') return;
...
async function generateSmartNotifications(config: NotificationConfig): Promise<SmartNotification[]> {
